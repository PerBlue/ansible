...
class EcsExecManager:
    ...
    def list_tasks(self, cluster_name, service_name, status):
        try:
            ...
            self.module.fail_json_aws(e, msg="Couldn't list ECS tasks")
    ...
    def run_task(self, cluster, task_definition, overrides, count, started_by):
        if overrides is None:
            overrides = dict()
        params = dict(cluster=cluster, taskDefinition=task_definition,
                      overrides=overrides, count=count, startedBy=started_by)
    ...
    def start_task(self, cluster, task_definition, overrides, container_instances, started_by):
        args = dict()
        if cluster:
    ...
    def stop_task(self, cluster, task):
        try:
            ...
            self.module.fail_json_aws(e, msg="Couldn't stop ECS task")
